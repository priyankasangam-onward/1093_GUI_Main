...
/* to delete error byte in receive buffer at MSB */
static uint8_t checkFlag = 0;// added flag to erase error  byte receving at MSB
...
int _modbus_receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
    ...
    while (length_to_read != 0) {
        ...
        /* this functionality is added to remove '00' extra bytes from receive buffer*/
        if(checkFlag == 1)
        {
            if(msg[0]==0 && rc != 0)
            {
...
// added by me ss modbus_boroadcast_mode
/* this functionality is added to write data in broadcast mode
 * working without response */
int modbus_broadcast_write_single(modbus_t *ctx, int function, int addr, int value)
{
    int rc;
    int req_length;
    uint8_t req[_MIN_REQ_LENGTH];
...
// added by me ss
/* this functionality is added for write data in broadcast mode in which response is not needed.
 * slave-id will be 0 for this */

int modbus_broadcast_mode(modbus_t *ctx, int addr, int nb, const uint16_t *src)
{
    int rc;
    int i;
    int req_length;
...
void _modbus_init_common(modbus_t *ctx)
{
    ...
    ctx->response_timeout.tv_usec = 0;
